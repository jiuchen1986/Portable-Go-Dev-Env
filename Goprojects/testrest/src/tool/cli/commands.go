...
//
// API "TEST REST API": CLI Commands
//
// Command:
// $ goagen
// --design=design
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "local-service",
		Short: `return the local service`,
	}
	...
	command = &cobra.Command{
		Use:   "service-chain",
		Short: `follow the service chain to the next service`,
	}
...
// Run makes the HTTP request corresponding to the LocalServiceTestServiceCommand command.
func (cmd *LocalServiceTestServiceCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = fmt.Sprintf("/api/%v/", url.QueryEscape(cmd.SvcLo))
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.LocalServiceTestService(ctx, path)
	if err != nil {
...
func (cmd *LocalServiceTestServiceCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var svcLo string
	cc.Flags().StringVar(&cmd.SvcLo, "svcLo", svcLo, `local service`)
}

// Run makes the HTTP request corresponding to the ServiceChainTestServiceCommand command.
func (cmd *ServiceChainTestServiceCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ServiceChainTestService(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *ServiceChainTestServiceCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var svcLo string
	cc.Flags().StringVar(&cmd.SvcLo, "svcLo", svcLo, `local service`)
	var svcOther string
